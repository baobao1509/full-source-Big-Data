...
try:
    print("--- Đang đọc file MARKETING từ ổ D ---")
    # Đường dẫn file CSV ở gốc ổ D
    df_loan = spark.read.option("header", "True").option("inferSchema", "True").csv("/data/bank-full.csv").limit(50000)
    ...
    for i in range(0, 50000, 1000):
        ...
        batch.select(to_json(struct([col(c) for c in batch.columns])).alias("value")) \
            .write.format("kafka").options(**kafka_options).option("topic", "bank_marketing").save()
